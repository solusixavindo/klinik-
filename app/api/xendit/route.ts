...
export async function GET() {
  return NextResponse.json({ ok: true, route: "xendit-webhook" })
}
...
export async function POST(req: Request) {
  try {
    ...
    const planCode = typeof event.metadata?.plan === "string"
      ? event.metadata.plan
      : getPlanFromExternalId(externalId)
    ...
    const message = err instanceof Error ? err.message : "Gagal memproses notifikasi"
